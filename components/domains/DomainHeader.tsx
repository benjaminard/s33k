...
const DomainHeader = (
   ...
   const isAiTraffic = router.pathname === '/domain/ai-traffic/[slug]';
   const isReadOnlyTab = isScoreboard || isAiTraffic;
   ...
   return (
      ...
      <div className='flex w-full justify-between mt-4 lg:mt-0'>
         <ul className=' max-w-[270px] overflow-auto flex items-end text-sm relative top-[2px] lg:max-w-none'>
            ...
            <li className={`${tabStyle} ${router.pathname === '/domain/ai-traffic/[slug]' ? 'bg-white border border-b-0 font-semibold' : ''}`}>
               <Link href={`/domain/ai-traffic/${domain.slug}`} passHref={true}>
                  <a className='px-4 py-2 inline-block'><Icon type="research" size={13} color='#999' classes='hidden lg:inline-block' />
                     <span className='text-xs lg:text-sm lg:ml-2'>AI Traffic</span>
                     <Icon
                     type='help'
                     size={14}
                     color="#aaa"
                     classes="ml-2 hidden lg:inline-block"
                     title='Which AI engines (ChatGPT, Perplexity, Claude...) are sending real visitors, from referral data'
                     />
                  </a>
               </Link>
            </li>
         ...
         <div className={'flex mb-0 lg:mb-1 lg:mt-3'}>
            {!isInsight && !isReadOnlyTab && <button className={`${buttonStyle} lg:hidden`} onClick={() => setShowOptions(!showOptions)}>
               <Icon type='dots' size={20} />
            </button>
            }
            {(isInsight || isReadOnlyTab) && <button className={`${buttonStyle} lg:hidden invisible`}>x</button>}
            <div
            ...
            style={{ display: showOptions ? 'block' : undefined }}>
               ...
               {!isConsole && !isInsight && !isIdeas && !isReadOnlyTab && (
                  <button
