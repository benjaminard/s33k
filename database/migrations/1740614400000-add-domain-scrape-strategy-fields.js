...
module.exports = {
   up: (queryInterface, Sequelize) => {
      return queryInterface.sequelize.transaction(async (t) => {
         ...
         if (!domainTableDefinition.scrape_strategy) {
            await queryInterface.addColumn('domain', 'scrape_strategy', { type: Sequelize.DataTypes.STRING, defaultValue: '' }, { transaction: t });
         }
         if (!domainTableDefinition.scrape_pagination_limit) {
            await queryInterface.addColumn(
               'domain', 'scrape_pagination_limit', { type: Sequelize.DataTypes.INTEGER, defaultValue: 0 }, { transaction: t },
            );
         }
         if (!domainTableDefinition.scrape_smart_full_fallback) {
            await queryInterface.addColumn(
               'domain', 'scrape_smart_full_fallback', { type: Sequelize.DataTypes.BOOLEAN, defaultValue: false }, { transaction: t },
            );
         }
      });
   },
